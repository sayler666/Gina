 ...
 */
open class FindNextDayAfterDateUseCase(val daysDataProvider: DaysDataProvider, val ormLiteErrorMapper: OrmLiteErrorMapper) {
    open fun findNextDayAfterDateId(dateTime: DateTime): Observable<Day> {
        val queryBuilder = daysDataProvider.dao.queryBuilder()
        val preparedQuery = queryBuilder
                .orderBy(Day.DATE_COL, true)
                .limit(1L)
                .where().gt(Day.DATE_COL, dateTime)
                .prepare()
